...
public class CursoModel2 extends AbstractTableModel {
	...
	private void atualizaIds() {  // <==
		...
		try {
			List<CadastroContratos> lista = dados.listaTodos();

			for (int i = 0; i < lista.size(); i++) {
				ids.put(i, lista.get(i).getidContrato());
			}
	...
	public Object getValueAt(int row, int col) {
		Object valor = null;
		try {
			CadastroContratos obj = dados.localiza(ids.get(row)); // <==
			...
			case 0:
				...
				break;
			case 1:
				valor = obj.getData().format(dateFormat);
	...
	public void setValueAt(Object value, int row, int col) {
		try {
			...
			case 0:
				...
				break;
			case 1:
				obj.setData(LocalDate.parse((String)value, dateFormat));
